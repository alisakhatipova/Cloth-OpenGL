...
float3      g_sunDir = normalize(float3(-0.5f,-1.0f, -0.5f));
float3 g_sunPos = float3(0, 0, 100);
...
GLUSboolean init(GLUSvoid)
{
  try 
  {
	...
	g_pLandTex         =new Texture2D("../data/wood.jpg");
	//g_pLandTex         =new Texture2D("../data/Ilya.bmp");
	
    g_pClothTex        = new Texture2D("../data/sponza_fabric_green_diff.tga");
	//g_pClothTex        = new Texture2D("../data/Ilya.bmp");
...
GLUSboolean update(GLUSfloat time)
{
  try 
  {
    ...
    //SimStep(&g_cloth, 0.1f);
	SimStep(&g_cloth, 5*time);
    ...
    setUniform(clothProgramId, "g_clothShaderId",  g_clothShaderId);
	...
	setUniform(g_program.program, "g_sunPos", g_sunPos);
	...
	setUniform(g_program.program, "g_sunPos", g_sunPos);
	bindTexture(g_program.program, 1, "LandTexture", g_pLandTex->GetColorTexId());
